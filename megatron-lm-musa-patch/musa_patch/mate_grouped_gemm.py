...
class _MateGroupedLinear(torch.autograd.Function):
    ...
    def forward(
        ...
        *weights: torch.Tensor,
        ...
        ctx.group_sizes_cpu = torch.tensor(ctx.m_splits, dtype=torch.int64)
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if any(ctx.needs_input_grad[4:]):
            ...
            use_tn_gm6 = (
                env_flag("MATE_TN_GM6_WGRAD", "0")
                and ctx.use_main_grad
                and num_experts > 1
                and all(ctx.needs_input_grad[4:])
                and all(size > 0 for size in ctx.m_splits)
            ...
            if use_tn_gm6:
                ...
                with torch.profiler.record_function("mate_tn_gm6_wgrad"):
