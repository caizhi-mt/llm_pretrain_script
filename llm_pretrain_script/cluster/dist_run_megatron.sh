...
# MATE expert fast path（默认启用；fprop/dgrad=MATE，wgrad 默认 TE）
MATE_GROUPED_GEMM="${MATE_GROUPED_GEMM:-1}"
MATE_USE_MAIN_GRAD="${MATE_USE_MAIN_GRAD:-1}"
MUTLASS_WGRAD="${MUTLASS_WGRAD:-0}"
MUTLASS_WGRAD_E32_K_GROUPED="${MUTLASS_WGRAD_E32_K_GROUPED:-1}"
MATE_FLASH_ATTN="${MATE_FLASH_ATTN:-1}"
MATE_CACHE_MUBIN_DISPATCH="${MATE_CACHE_MUBIN_DISPATCH:-1}"
MATE_DEFER_DEEPEP_COUNTS="${MATE_DEFER_DEEPEP_COUNTS:-1}"
MUSA_COMPACT_PERMUTE="${MUSA_COMPACT_PERMUTE:-1}"
MUSA_FUSED_MLA_DOWN_PROJ="${MUSA_FUSED_MLA_DOWN_PROJ:-1}"
MUSA_NATIVE_ROPE="${MUSA_NATIVE_ROPE:-1}"
MUSA_FUSED_MLA_ROPE="${MUSA_FUSED_MLA_ROPE:-1}"
# MATE Python 提交线程绑核（默认关闭；map 含分号，远端必须加引号）
...
for host in ${ip_list[@]}; do
  echo "$host"
  ssh -f -n "$host" "bash -c 'cd \"${WORK_DIR}\"; WORLD_SIZE=${NNODES} RANK=${COUNT} MASTER_ADDR=${MASTER_ADDR} MASTER_PORT=${MAIN_PROCESS_PORT} TRAINING_STEPS=${TRAINING_STEPS} LOG_NAME=${LOG_NAME} ENABLE_TENSORBOARD=${ENABLE_TENSORBOARD} MUSA_PRETRAIN_ENTRY=${MUSA_PRETRAIN_ENTRY:-} ENABLE_SEQUENCE_PARALLEL=${ENABLE_SEQUENCE_PARALLEL:-} ENABLE_PROFILER=${ENABLE_PROFILER:-} PROFILER_FREQ=${PROFILER_FREQ:-} PROFILER_WARMUP_STEPS=${PROFILER_WARMUP_STEPS:-} PROFILER_PROFILE_MEMORY=${PROFILER_PROFILE_MEMORY:-} MUSA_LAUNCH_BLOCKING=${MUSA_LAUNCH_BLOCKING:-} PROFILE_STEP_START=${PROFILE_STEP_START:-} PROFILE_STEP_END=${PROFILE_STEP_END:-} USE_DEEPEP_ACE=${USE_DEEPEP_ACE:-} MOE_GROUPED_GEMM=${MOE_GROUPED_GEMM:-} MATE_GROUPED_GEMM=${MATE_GROUPED_GEMM} MATE_USE_MAIN_GRAD=${MATE_USE_MAIN_GRAD} MUTLASS_WGRAD=${MUTLASS_WGRAD} MUTLASS_WGRAD_E32_K_GROUPED=${MUTLASS_WGRAD_E32_K_GROUPED} MATE_FLASH_ATTN=${MATE_FLASH_ATTN} MATE_CACHE_MUBIN_DISPATCH=${MATE_CACHE_MUBIN_DISPATCH} MATE_DEFER_DEEPEP_COUNTS=${MATE_DEFER_DEEPEP_COUNTS} MUSA_COMPACT_PERMUTE=${MUSA_COMPACT_PERMUTE} MUSA_FUSED_MLA_DOWN_PROJ=${MUSA_FUSED_MLA_DOWN_PROJ} MUSA_NATIVE_ROPE=${MUSA_NATIVE_ROPE} MUSA_FUSED_MLA_ROPE=${MUSA_FUSED_MLA_ROPE} MUSA_CPU_AFFINITY=${MUSA_CPU_AFFINITY} MUSA_CPU_AFFINITY_MODE=${MUSA_CPU_AFFINITY_MODE} MUSA_CPU_AFFINITY_MAP=\"${MUSA_CPU_AFFINITY_MAP}\" bash ${DIST_TRAIN} > ${LOG_DIR}/log.${COUNT}.${host} 2>&1 &'"
