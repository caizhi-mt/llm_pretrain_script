...
| `MUTLASS_WGRAD` | `0` | 实验性启用已验证 E128 小-M，以及 E32 `[4096,7168]` FC1 的 MP31 MUTLASS beta=1 wgrad；首 microbatch、FC2 和其他 shape 自动走 TE。 |
| `MUTLASS_WGRAD_E32_K_GROUPED` | `1` | E32 FC1 stage4 按动态 reduction-K 排序并每 4 个 expert 合并 launch；设为 `0` 回退逐 expert stage4，用于 A/B。 |
...
- 实现：不再使用 Triton。MP31 MUTLASS 直接把 packed BF16 `dY.T @ X` 累加到
  ...
  使用 `256×384×32, stage=4`，并按动态 reduction-K 降序每 4 个 expert 合并 launch。
  三者均使用 `KernelTme + WithTme` epilogue。动态 counts 和所有 tensor pointer 每次
  调用重新传入，不缓存路由、tensor 或输出。
- Dispatch 与回退：新增 `MUTLASS_WGRAD=0`，默认关闭。设为 `1` 后在 E128
  `[N,K]=[1536,2048]`/`[2048,768]` 小-M，以及 E32 `[4096,7168]` FC1 的已验证
  范围内生效，且都要求 BF16 与 beta=1 FP32 main-grad 累加。首个 microbatch、
  E32 FC2、非 main-grad 和未验证 shape 继续走原 TE。
- 精度：独立/非连续 output tensor、零 token expert、奇数且非均匀 counts 均通过；
  ...
  non-default stream。新增 E32 group4 后，目标测试 `11 passed, 4 skipped`，显式 MP31
  测试 `10 passed`，覆盖跨 grouped launch、零 expert 和 non-default stream。另有
  Python/native 双层 pointer alignment 和 MP31 capability guard。
...
- 构建：启动时使用 MATE 0.2.5 wheel 内置 MUTLASS JIT 编译，cache key 包含源码、
  ...
  loader 生成的 `--compiler-options -fPIC`，并支持 `MUSA_HOME`/`MUSA_PATH`。E32 多
  microbatch 尚未做完整训练/trace A/B；在此之前保持默认关闭，不能把算子收益等同于生产
  step-time 收益。
